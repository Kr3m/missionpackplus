...
line 982
;979:
;980:	// battlesuit protects from all radius damage (but takes knockback)
;981:	// and protects 50% against all damage
;982:	if ( client && client->ps.powerups[PW_BATTLESUIT] ) {
ADDRLP4 0
INDIRP4
CVPU4 4
CNSTU4 0
...
EQI4 $423
line 983
;983:		G_AddEvent( targ, EV_POWERUP_BATTLESUIT, 0 );
ADDRFP4 0
INDIRP4
ARGP4
CNSTI4 62
...
pop
line 984
;984:		if ( ( dflags & DAMAGE_RADIUS ) || ( mod == MOD_FALLING ) ) {
ADDRFP4 24
INDIRI4
CNSTI4 1
BANDI4
...
line 1002
;1000:
;1001:	//qlone - self damages
;1002:	if ( targ == attacker && g_noSelfDamage.integer )
ADDRFP4 0
INDIRP4
CVPU4 4
ADDRFP4 8
...
;1004:	else {
line 1007
;1005:	//qlone - self damages
;1006:		// save some from armor
;1007:		asave = CheckArmor( targ, take, dflags );
ADDRFP4 0
INDIRP4
ARGP4
ADDRLP4 8
...
ASGNI4
line 1009
;1009:	} //qlone - self damages
LABELV $433
line 1011
;1010:
;1011:	if ( g_debugDamage.integer ) {
ADDRGP4 g_debugDamage+12
INDIRI4
CNSTI4 0
EQI4 $435
line 1012
;1012:		G_Printf( "%i: client:%i health:%i damage:%i armor:%i\n", level.time, targ->s.number,
ADDRGP4 $438
ARGP4
ADDRGP4 level+32
INDIRI4
...
pop
line 1014
;1013:			targ->health, take, asave );
;1014:	}
LABELV $435
line 1017
;1015:
;1016:	// add to the attacker's hit counter (if the target isn't a general entity like a prox mine)
;1017:	if ( attacker->client && client && targ != attacker && targ->health > 0
ADDRLP4 56
ADDRFP4 8
INDIRP4
ASGNP4
...
EQI4 $440
line 1019
;1018:			&& targ->s.eType != ET_MISSILE
;1019:			&& targ->s.eType != ET_GENERAL) {
line 1021
...
line 1038
;1027:#else
;1028:		// we may hit multiple targets from different teams
;1029:		// so usual PERS_HITS increments/decrements could result in ZERO delta
;1030:		if ( OnSameTeam( targ, attacker ) ) {
;1031:			attacker->client->damage.team++;
;1032:		} else {
;1033:			attacker->client->damage.enemy++;
;1034:			// accumulate damage during server frame
;1035:			attacker->client->damage.amount += take + asave;
;1036:		}
;1037:#endif
;1038:	}
LABELV $440
line 1043
;1039:
;1040:	// add to the damage inflicted on a player this frame
;1041:	// the total will be turned into screen blends and view angle kicks
;1042:	// at the end of the frame
;1043:	if ( client ) {
ADDRLP4 0
INDIRP4
CVPU4 4
...
EQU4 $444
line 1044
;1044:		if ( attacker ) { // FIXME: always true?
ADDRFP4 8
INDIRP4
CVPU4 4
CNSTU4 0
EQU4 $446
line 1045
;1045:			client->ps.persistant[PERS_ATTACKER] = attacker->s.number;
ADDRLP4 0
INDIRP4
CNSTI4 272
ADDP4
...
LABELV $446
line 1047
;1047:			client->ps.persistant[PERS_ATTACKER] = ENTITYNUM_WORLD;
ADDRLP4 0
INDIRP4
CNSTI4 272
ADDP4
...
LABELV $447
line 1049
;1049:		client->damage_armor += asave;
ADDRLP4 64
ADDRLP4 0
INDIRP4
CNSTI4 688
...
ASGNI4
line 1051
;1051:		client->damage_knockback += knockback;
ADDRLP4 72
ADDRLP4 0
INDIRP4
CNSTI4 696
...
EQU4 $448
line 1053
;1053:			VectorCopy ( dir, client->damage_from );
ADDRLP4 0
INDIRP4
CNSTI4 700
ADDP4
...
ASGNB 12
line 1054
;1054:			client->damage_fromWorld = qfalse;
ADDRLP4 0
INDIRP4
CNSTI4 712
ADDP4
...
LABELV $448
line 1056
;1056:			VectorCopy ( targ->r.currentOrigin, client->damage_from );
ADDRLP4 0
INDIRP4
CNSTI4 700
ADDP4
...
ASGNB 12
line 1057
;1057:			client->damage_fromWorld = qtrue;
ADDRLP4 0
INDIRP4
CNSTI4 712
ADDP4
...
line 1063
;1060:
;1061:	// See if it's the player hurting the emeny flag carrier
;1062:#ifdef MISSIONPACK
;1063:	if( g_gametype.integer == GT_CTF || g_gametype.integer == GT_1FCTF ) {
ADDRGP4 g_gametype+12
INDIRI4
CNSTI4 4
EQI4 $454
...
line 1067
;1064:#else	
;1065:	if( g_gametype.integer == GT_CTF) {
;1066:#endif
;1067:		Team_CheckHurtCarrier(targ, attacker);
ADDRFP4 0
INDIRP4
ARGP4
ADDRFP4 8
...
EQU4 $455
line 1072
;1071:		// set the last client who damaged the target
;1072:		targ->client->lasthurt_client = attacker->s.number;
ADDRFP4 0
INDIRP4
CNSTI4 516
ADDP4
...
ASGNI4
line 1073
;1073:		targ->client->lasthurt_mod = mod;
ADDRFP4 0
INDIRP4
CNSTI4 516
ADDP4
...
line 1077
;1075:
;1076:	//qlone - self damages
;1077:	if ( targ == attacker && g_noSelfDamage.integer )
ADDRFP4 0
INDIRP4
CVPU4 4
ADDRFP4 8
...
EQI4 $460
line 1083
;1083:		targ->health = targ->health - take;
ADDRLP4 64
ADDRFP4 0
INDIRP4
ASGNP4
...
EQU4 $462
line 1085
;1085:			targ->client->ps.stats[STAT_HEALTH] = targ->health;
ADDRLP4 68
ADDRFP4 0
INDIRP4
ASGNP4
...
EQU4 $466
line 1090
;1090:				targ->flags |= FL_NO_KNOCKBACK;
ADDRLP4 68
ADDRFP4 0
INDIRP4
CNSTI4 536
...
ASGNP4
line 1096
;1096:			targ->die (targ, inflictor, attacker, take, mod);
ADDRLP4 72
ADDRFP4 0
INDIRP4
ASGNP4
...
EQU4 $470
line 1099
;1099:			targ->pain (targ, attacker, take);
ADDRLP4 68
ADDRFP4 0
INDIRP4
ASGNP4
...
;1108:CanDamage
;1109:
;1110:Returns qtrue if the inflictor can directly damage the target.  Used for
;1111:explosions and melee attacks.
;1112:============
;1113:*/
;1114:qboolean CanDamage( gentity_t *targ, vec3_t origin )
;1115:{
line 1123
;1116:	//we check if the attacker can damage the target, return qtrue if yes, qfalse if no
;1117:	vec3_t	dest;
;1118:	trace_t	tr;
;1119:	vec3_t	midpoint;
;1120:	vec3_t				size;
;1121:
;1122:	// use the midpoint of the bounds instead of the origin, because bmodels may have their origin 0,0,0
...
ASGNF4
line 1124
;1124:	VectorScale( midpoint, 0.5, dest );
ADDRLP4 0
ADDRLP4 80
INDIRF4
CNSTF4 1056964608
...
pop
line 1127
;1127:	if (tr.fraction == 1.0 || tr.entityNum == targ->s.number)
ADDRLP4 12+8
INDIRF4
CNSTF4 1065353216
EQF4 $483
...
line 1130
;1129:
;1130:	VectorSubtract( targ->r.absmax, targ->r.absmin, size );
ADDRLP4 104
ADDRFP4 0
INDIRP4
ASGNP4
...
;1134:	// - +
;1135:	// - -
;1136:	VectorCopy( targ->r.absmax, dest );
ADDRLP4 0
ADDRFP4 0
INDIRP4
CNSTI4 476
...
ASGNB 12
line 1137
;1137:	trap_Trace ( &tr, origin, vec3_origin, vec3_origin, dest, ENTITYNUM_NONE, MASK_SOLID);
ADDRLP4 12
ARGP4
ADDRFP4 4
INDIRP4
...
ASGNF4
line 1144
;1144:	trap_Trace( &tr, origin, vec3_origin, vec3_origin, dest, ENTITYNUM_NONE, MASK_SOLID );
ADDRLP4 12
ARGP4
ADDRFP4 4
INDIRP4
...
ASGNF4
line 1151
;1151:	trap_Trace( &tr, origin, vec3_origin, vec3_origin, dest, ENTITYNUM_NONE, MASK_SOLID );
ADDRLP4 12
ARGP4
ADDRFP4 4
INDIRP4
...
ASGNF4
line 1158
;1158:	trap_Trace( &tr, origin, vec3_origin, vec3_origin, dest, ENTITYNUM_NONE, MASK_SOLID );
ADDRLP4 12
ARGP4
ADDRFP4 4
INDIRP4
...
;1164:	// - -
;1165:	// + -
;1166:	VectorCopy( targ->r.absmin, dest );
ADDRLP4 0
ADDRFP4 0
INDIRP4
CNSTI4 464
...
ASGNB 12
line 1167
;1167:	trap_Trace ( &tr, origin, vec3_origin, vec3_origin, dest, ENTITYNUM_NONE, MASK_SOLID);
ADDRLP4 12
ARGP4
ADDRFP4 4
INDIRP4
...
ASGNF4
line 1174
;1174:	trap_Trace ( &tr, origin, vec3_origin, vec3_origin, dest, ENTITYNUM_NONE, MASK_SOLID);
ADDRLP4 12
ARGP4
ADDRFP4 4
INDIRP4
...
ASGNF4
line 1181
;1181:	trap_Trace ( &tr, origin, vec3_origin, vec3_origin, dest, ENTITYNUM_NONE, MASK_SOLID);
ADDRLP4 12
ARGP4
ADDRFP4 4
INDIRP4
